...
const query = ({
  ...
  const subGraph = evaluatedGraph.value.filter(({
    value
  }) => matches.some(fn => fn(value)));
  return {
    type: 'graph',
    value: subGraph,
    closure: scope
...
const mutation = ({
  ...
  return {
    type: 'graph',
    value: (0, _lodash.concat)(evaluatedGraph.value, evaluatedPattern.subPatterns),
    closure: (0, _lodash.merge)(evaluatedGraph.closure, evaluatedPattern.closure)
  };
};
